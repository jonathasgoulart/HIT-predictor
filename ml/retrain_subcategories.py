"""
Retreinamento com Subcategorias
Treina modelos para as novas subcategorias criadas
"""
import sys
...
def train_subcategory_model(genre, dataset_path):
    """Treina modelo para subcategoria"""
    print(f"\n{'='*70}")
    ...
    if len(df) < 20:
        print(f"AVISO: Dataset muito pequeno ({len(df)} musicas)")
        print("Minimo recomendado: 20 musicas")
    ...
    cv_scores = cross_val_score(model, X, y, cv=min(5, len(df)//10))
...
def main():
    """Treina todas as subcategorias"""
    print("\n" + "="*70)
    print("RETREINAMENTO COM SUBCATEGORIAS")
    print("="*70)
    
    datasets_dir = Path(__file__).parent / 'datasets'
    
    # Subcategorias para treinar
    subcategories = {
        'nova_mpb': 'nova_mpb_ml.csv',
        'rnb_trap': 'rnb_trap_ml.csv',
        'rnb_pop': 'rnb_pop_ml.csv'
        # mpb_rock tem apenas 3 musicas, muito pequeno
    }
    ...
    for genre, filename in subcategories.items():
        ...
        try:
            accuracy, model_path = train_subcategory_model(genre, str(dataset_path))
    ...
    # Relatório
    print("\n" + "="*70)
    print("RELATORIO FINAL - SUBCATEGORIAS")
    print("="*70 + "\n")
